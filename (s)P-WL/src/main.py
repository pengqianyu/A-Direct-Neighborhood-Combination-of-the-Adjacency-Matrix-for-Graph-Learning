...
def igraph_edegs(graph):

    max_all=0

    for i in range(len(graph.es)):

        max_value = max(graph.es[i].tuple)

        if max_value > max_all:
    ...
    for i in range(max_all):
        ...
        for j in range(len(graph.es)):

            if i in graph.es[j].tuple:

                if graph.es[j].tuple[0] == i:

                    chosen_id_1 = graph.es[j].tuple[1]

                else:

                    chosen_id1 = graph.es[j].tuple[0]

            for k in range(len(graph.es)):

                if chosen_id_1 in graph.es[k].tuple and not (i in graph.es[k].tuple):

                    if graph.es[k].tuple[0] == chosen_id_1:

                        chosen_id_2 = graph.es[k].tuple[1]

                    else:

                        chosen_id_2 = graph.es[k].tuple[0]

                for len1 in range(len(add_edges_matrix)):

                    if chosen_id_1 in add_edges_matrix[len1] and chosen_id_2 in add_edges_matrix[len2]:

                        already = True

                    if not already:

                        add_edges_matrix.append([chosne_id_1,chosen_id_2])

        for i in range(len(add_edges_matrix)):

            graph.add_edge(add_edges_matrix[i][0],add_edges_matrix[i][1])
